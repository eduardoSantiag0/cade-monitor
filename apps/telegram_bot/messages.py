...
Emojis por tema (sempre os mesmos para o mesmo significado):
  ⚖️ processo · 🧾 documento · 📡 monitoramento · 🔍/🔎 consulta ao SEI · ⏳ aguarde
  📬 alerta · 🆕 mudança · 🗒️ movimentações · 🕒 horários · 🔗 link
  🔕/🔔 pausar/retomar · 📚 histórico · 📋 lista · 🔒 sem acesso · 🛡️ admin
"""
...
COMMANDS_HELP = (
    ...
    '⚖️ /status <processo> — situação e última movimentação\n'
    '🧾 /last_update <processo> — última atualização com o documento\n'
...
def missing_arg(command: str) -> str:
    return f'✍️ Informe o número do processo. Ex.: /{command} {EXAMPLE}'


def invalid_ref() -> str:
    return f'⚠️ Número inválido. Use o formato {EXAMPLE} ou o link público do processo no SEI.'
...
def admin_only(command: str) -> str:
    return f'🛡️ Neste grupo, só administradores podem usar /{command}.'


def admin_check_failed() -> str:
    return '🛡️ Não consegui confirmar se você é administrador do grupo. Tente de novo em instantes.'
...
def already_following(label: str) -> str:
    return f'📌 Você já acompanha ⚖️ {label}. Use /status para ver a situação.'


def watch_queued(ref: str) -> str:
    return f'🔍 Consultando o processo {ref} pela primeira vez no SEI.\n⏳ Aviso aqui em instantes.'


def watch_started(label: str) -> str:
    return (
        f'✅ Pronto! Agora acompanho ⚖️ {label}.\n'
        '📬 Você recebe um aviso aqui a cada movimentação nova.\n'
        '🧾 A última atualização, com o documento, vem logo abaixo.'
    )


def latest_block(records: list[str]) -> str:
    if not records:
        return '🗒️ Ainda não identifiquei movimentações na página do processo.'
    return '🗒️ Últimas movimentações:\n' + '\n'.join(f'▫️ {r}' for r in records)
...
def watch_retrying(ref: str) -> str:
    return (
        f'⏳ O SEI não respondeu ao consultar {ref}. Seu pedido está registrado e vou tentar '
...
def unwatched(label: str) -> str:
    return f'🗂️ Você não acompanha mais ⚖️ {label}.'
...
def status(label: str, url: str, last_change, last_checked, records: list[str], paused: bool) -> str:
    ...
    paused_line = '\n🔕 Alertas pausados para você.' if paused else ''
    return (
        f'⚖️ {label}\n'
...
def history(label: str, changes: list) -> str:
    ...
    lines = [f'📚 Últimas movimentações de ⚖️ {label}:']
    for change in changes:
        lines.append(f'\n🗓️ {fmt_dt(change.detected_at)}\n{change.summary}')
    return '\n'.join(lines)


def paused(label: str) -> str:
    return f'🔕 Alertas de ⚖️ {label} pausados. Use /resume para retomar.'


def resumed(label: str) -> str:
    return f'🔔 Alertas de ⚖️ {label} retomados.'


def check_cooldown(label: str, last_checked, minutes_left: int, records: list[str]) -> str:
    return (
        f'⏱️ {label} foi verificado há pouco ({fmt_dt(last_checked)}). '
        f'Você pode pedir nova verificação em {minutes_left} min.\n\n{latest_block(records)}'
    )


def check_queued(label: str) -> str:
    return f'🔎 Verificando ⚖️ {label} no SEI agora.\n⏳ Aviso aqui em instantes.'


def check_no_change(label: str, records: list[str]) -> str:
    return f'✅ Sem novidades em ⚖️ {label}.\n\n{latest_block(records)}'


def check_changed(label: str) -> str:
    # O alerta completo sai pelo fluxo normal de notificações, logo em seguida.
    return f'🚨 Encontrei movimentação nova em ⚖️ {label}!\n📬 Os detalhes chegam no alerta a seguir.'
...
def latest_queued(label: str) -> str:
    return f'🧾 Preparando a última atualização de ⚖️ {label}, com o documento.\n⏳ Envio em instantes.'


def latest_no_data(label: str) -> str:
    return (
        f'⏳ Ainda não tenho a primeira leitura de {label}. '
        'Assim que ela sair, use /last_update de novo.'
    )
